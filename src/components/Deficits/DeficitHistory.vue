          ...
          <table class="text-sm w-full mt-3 overflow-auto">
            ...
            <tbody>
              <tr v-for="(item, index) in deficits" :key="index">
                ...
                <td class="px-2 pt-1 text-center">{{ item?.username }}</td>
...
export default {
  ...
  setup() {
    ...
    const dis = ref(false);
    const meta = ref({});
    const tableHead = ["ID", "USERNAME", "DATE", "TIME", "AMOUNT", "BITCOIN"];

    const getDeficits = async (page_size, page = 1) => {
      try {
        const { data } = await customFetcher(
          `deficit/get/all/${page_size}/${page}`
        );
        deficits.value = data.items;
        meta.value = data.meta;
        noneText.value = "There hasn't been any deficits yet!";
      } catch {}
    };
    ...
    const searchDeficits = async (index) => {
      try {
        if (index && index != 0) {
          const { data } = await customFetcher(`deficit/get/all/${index}`);
          if (data?.id) {
            deficits.value = [data];
          }
          noneText.value = "";
